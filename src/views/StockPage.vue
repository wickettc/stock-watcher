...
export default {
    ...
    computed: {
        ...
        timeLineSeries() {
            const newHigh = this.timeSeries.map(item => parseFloat(item.high).toFixed(2))
            const newOpen = this.timeSeries.map(item => parseFloat(item.open).toFixed(2))
            const newLow = this.timeSeries.map(item => parseFloat(item.low).toFixed(2))
            const newClose = this.timeSeries.map(item => parseFloat(item.close).toFixed(2))
            return [
                {
                name: "High",
                data: newHigh
        ...
        timeLineChartOptions() {
            const newDays = this.timeSeries.map(item => item.datetime.slice(5))
            const findMin = Math.min(...this.timeSeries.map(item => item.low)) - 1;
            const newMin = findMin <= 0 ? 0 : Math.floor(findMin)
            const newMax = Math.floor(Math.max(...this.timeSeries.map(item => item.high)) + 1);
            return {
                chart: {
              height: 350,
              type: 'line',
